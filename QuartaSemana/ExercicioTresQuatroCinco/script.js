...
function checkEveryoneHasProgrammerOccupation( listPeople ){
    checkList( listPeople );
...
function getPeopleUnderAge( listPeople ){
    ...
    let peopleUnderAge = listPeople.filter( person => person.age < 18 && person.age >= 0 );

    if( !peopleUnderAge ){
        return console.log( "Não tem pessoas menores que 18 anos na lista.");
    }

    return peopleUnderAge;
}
